...
> **Note**: The examples resolve `ballerinax/microsoft.excel` the same way any consumer
> does. Until this version is published to Ballerina Central, stage it locally first with
> `./build.sh build` (see [Building the examples with the local module](#building-the-examples-with-the-local-module));
> otherwise the commands below resolve the previously published version and fail to compile.
